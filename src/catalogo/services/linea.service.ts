...
import type { LineaRepositoryInterface } from '../repositories/interfaces/linea.repository.interface';
...
export class LineaService implements LineaServiceInterface {
  ...
  async findOneActive(id: number): Promise<Linea> {
    ...
    if (!linea) {
      throw new NotFoundException(
        `Línea con ID ${id} no encontrada o está eliminada.`,
      );
    }
    return linea;
  }
  ...
  async create(data: CreateLineaDto): Promise<Linea> {
    ...
    const existingActiveLinea = await this.lineaRepository.findByName(
      data.nombre,
    );
    if (existingActiveLinea) {
      throw new BadRequestException(
        `El nombre de línea '${data.nombre}' ya existe en el sistema.`,
      );
    }

    // 2. Verificar si existe una línea eliminada con el mismo nombre
    const existingDeletedLinea = await this.lineaRepository.findByName(
      data.nombre,
      true,
    );
  ...
  async update(id: number, data: UpdateLineaDto): Promise<Linea> {
    ...
    if (data.nombre) {
      ...
      if (existingLinea && existingLinea.id !== id) {
        throw new BadRequestException(
          `El nombre de línea '${data.nombre}' ya está siendo usado por otra línea.`,
        );
      }
    }
