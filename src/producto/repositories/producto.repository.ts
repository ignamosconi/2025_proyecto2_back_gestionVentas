...
export class ProductoRepository implements ProductoRepositoryInterface {
  ...
  async create(
    data: CreateProductoDto | DeepPartial<Producto>,
  ): Promise<Producto> {
  ...
  async update(idProducto: number, data: UpdateProductoDto): Promise<Producto> {
    ...
    if (result.affected === 0) {
      throw new NotFoundException(
        `No se pudo actualizar el producto con ID ${idProducto} porque no existe.`,
      );
    }
    ...
    if (!updatedProducto) {
      throw new NotFoundException(
        `Producto con ID ${idProducto} no encontrado después de la actualización.`,
      );
    }
  ...
  async softDelete(idProducto: number): Promise<void> {
    ...
    if (result.affected === 0) {
      throw new NotFoundException(
        `Producto con ID ${idProducto} no encontrado o ya estaba eliminado lógicamente.`,
      );
    }
  }
  ...
  async restore(idProducto: number): Promise<void> {
    ...
    if (result.affected === 0) {
      throw new NotFoundException(
        `Producto con ID ${idProducto} no encontrado o no estaba eliminado lógicamente.`,
      );
    }
  }
  ...
  async updateStock(idProducto: number, change: number): Promise<Producto> {
    ...
    if (!producto) {
      throw new NotFoundException(
        `Producto con ID ${idProducto} no encontrado o eliminado.`,
      );
    }
    ...
    if (nuevoStock < 0) {
      console.warn(
        `(!) El producto ID ${idProducto} quedó con stock negativo (${nuevoStock}).`,
      );
    }
