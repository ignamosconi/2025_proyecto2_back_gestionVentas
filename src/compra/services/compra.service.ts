...
export class CompraService implements CompraServiceInterface {
  ...
  async create(
    ...
    for (const item of detalles) {
      ...
      if (!producto)
        {throw new NotFoundException(
          `Producto con ID ${item.idProducto} no encontrado o está inactivo.`,
        );}
    ...
    try {
      ...
      const compraRecargada = await this.compraRepository.findOne(
      ...
      return plainToInstance(CompraResponseDto, compraRecargada, {
      ...
      throw new BadRequestException(
        'Error al registrar la compra y actualizar el inventario: ' +
          error.message,
      );
    } finally {
  ...
  async findAll(): Promise<CompraResponseDto[]> {
    ...
    return plainToInstance(CompraResponseDto, compras, {
      excludeExtraneousValues: true,
    });
  }
  ...
  async findOne(id: number): Promise<CompraResponseDto> {
    ...
    return plainToInstance(CompraResponseDto, compra, {
      excludeExtraneousValues: true,
    });
  }
  ...
  async update(
    ...
    const {
      metodoPago,
      detalles: nuevosDetallesDto,
      idProveedor,
    } = updateCompraDto as any;
    ...
    try {
      ...
      if (nuevosDetallesDto?.length) {
        for (const nuevoDetalleDto of nuevosDetallesDto) {
          ...
          if (idProducto === undefined || cantidadNueva === undefined) {
            throw new BadRequestException(
              'El idProducto y la cantidad son obligatorios para cada detalle de compra.',
            );
          }
          ...
          const detalleExistente = detallesActuales.find(
            (d) => (d as any).idProducto === idProducto,
          );
          const producto =
      ...
      return plainToInstance(CompraResponseDto, compraActualizada, {
        excludeExtraneousValues: true,
      });
    } catch (error) {
      await queryRunner.rollbackTransaction();
      if (
        ...
        {throw error;}
      throw new BadRequestException(
        'Error al actualizar la compra y el inventario: ' + error.message,
      );
    } finally {
      await queryRunner.release();
    }
