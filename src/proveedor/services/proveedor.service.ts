...
export class ProveedorService implements ProveedorServiceInterface {
  ...
  async create(data: CreateProveedorDto): Promise<Proveedor> {
    ...
    const existingProveedor = await this.proveedorRepository.findByNombreActive(
      data.nombre,
    );
  ...
  async update(id: number, data: UpdateProveedorDto): Promise<Proveedor> {
    ...
    if (data.nombre && data.nombre !== proveedor.nombre) {
      const existingProveedor =
        await this.proveedorRepository.findByNombreActive(data.nombre);
  ...
  async softDelete(id: number): Promise<void> {
    ...
    const relaciones =
      await this.productoProveedorRepository.findByProveedor(id);

    // Eliminar (soft delete) todas las relaciones
    for (const relacion of relaciones) {
      await this.productoProveedorRepository.softDelete(
        relacion.idProductoProveedor,
      );
    }
  ...
  async findOneActive(id: number): Promise<Proveedor> {
    ...
    if (!proveedor || proveedor.deletedAt)
      {throw new NotFoundException(
        `Proveedor con id ${id} no encontrado o eliminado`,
      );}
    return proveedor;
  }
}
