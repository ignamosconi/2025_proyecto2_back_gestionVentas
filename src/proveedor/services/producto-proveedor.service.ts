...
export class ProductoProveedorService
  implements ProductoProveedorServiceInterface
{
  constructor(
  ...
  async findOne(id: number): Promise<ProductoProveedor> {
    ...
    if (!entity)
      {throw new NotFoundException(
        `ProductoProveedor con id ${id} no encontrado`,
      );}
    return entity;
  }
  ...
  async create(data: CreateProductoProveedorDto): Promise<ProductoProveedor> {
    ...
    const existingRelation =
      await this.productoProveedorRepo.findExistingRelation(
        data.idProducto,
        data.idProveedor,
      );
    ...
    if (existingRelation) {
      if (existingRelation.deletedAt) {
        ...
        await this.productoProveedorRepo.restore(
          existingRelation.idProductoProveedor,
        );
        // Actualizar el código del proveedor
        return this.productoProveedorRepo.update(
          existingRelation.idProductoProveedor,
          {
  ...
  async checkLinkExists(
    ...
    return this.productoProveedorRepo.findOneByProductAndSupplier(
      idProducto,
      idProveedor,
    );
  }
