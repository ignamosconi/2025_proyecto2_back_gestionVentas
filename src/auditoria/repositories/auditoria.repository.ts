...
export class AuditoriaRepository implements IAuditoriaRepository {
  ...
  async findAuditLogs(
    ...
    if (tipo_evento) qb.andWhere('audit.tipo_evento = :tipo_evento', { tipo_evento });
    if (fechaDesde) qb.andWhere('audit.fecha_hora >= :fechaDesde', { fechaDesde });
    if (fechaHasta) qb.andWhere('audit.fecha_hora <= :fechaHasta', { fechaHasta });
