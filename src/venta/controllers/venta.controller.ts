import { Controller, Get, Param, Post, Body, Req, UseGuards, HttpCode, HttpStatus, Patch, Put, Logger } from '@nestjs/common';
...
  findByUsuario(@Param('idUsuario') idUsuario: number): Promise<VentaResponseDto[]> {
...
  create(@Body() createVentaDto: CreateVentaDto, @Req() req: any): Promise<VentaResponseDto> {
    const userId = req.user.id; // usuario autenticado
    this.logger.log(`--- ENTRADA A create (POST) ---`);
    this.logger.debug(`ID de Usuario Autenticado: ${userId}`);
    this.logger.debug(`Body (DTO de Creación): ${JSON.stringify(createVentaDto)}`);
    return this.ventaService.create(createVentaDto, userId);
  }

  @Put(':id')
  @Roles(UserRole.EMPLOYEE, UserRole.OWNER)
  @ApiParam({ name: 'id', type: Number, description: 'ID de la venta a actualizar' })
...
  ): Promise<VentaResponseDto> {
    ...
    this.logger.debug(`Body (DTO de Actualización): ${JSON.stringify(updateVentaDto)}`);
